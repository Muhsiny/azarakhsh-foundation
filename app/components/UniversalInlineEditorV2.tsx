...
export default function UniversalInlineEditorV2() {
  ...
  useEffect(() => {
    ...
    const scan = () => {
      ...
      const all = Array.from(document.querySelectorAll<HTMLElement>(editableSelector)).filter((el) => !el.closest("[data-inline-ui]") && !el.closest("[data-inline-static]") && !el.closest("[data-custom-subsection]") && !el.closest(".azarakhsh-approved"));
      all.forEach((element) => {
