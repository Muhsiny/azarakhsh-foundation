...
export default async function ContactPage() {
  ...
  return <InstitutionalPage kicker="ارتباط با بنیاد" title="تماس و همکاری با بنیاد" lead="این صفحه برای پرسش، همکاری پژوهشی، درخواست اصلاح و امور حقوق نشر است. ارسال سند و خاطره مسیر جداگانه‌ای دارد." sections={sections} intro={<div className="az-contact-cards"><div><h2>ارتباط مستقیم با بنیاد</h2><address><a href={`mailto:${settings.contact.email}`} dir="ltr">{settings.contact.email}</a></address></div><div><h2>سند یا خاطره‌ای دارید؟</h2><p>فرم ارسال منبع، اطلاعات منشأ، پیوست و ترجیح شما دربارهٔ ذکر نام را یک‌جا ثبت می‌کند.</p><a className="az-action" href="/contribute">رفتن به فرم ارسال سند و خاطره ←</a></div></div>} />;
}
